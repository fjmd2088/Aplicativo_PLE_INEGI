...
namespace App_PLE.Vistas
{
    public partial class FormRegistros: Form
    {
        ...
        private void btnGuardarIniUO_Click(object sender, EventArgs e)
        {
            bool cv = ValidarCampos_UO();
            //bool cv = true;
        ...
        private bool ValidarCampos_UO()
        {
            // Array de controles a validar
            Control[] controlesAValidar = {
        txt_turno_iniciativa_urgente_obvia, cmb_estatus_iniciativa_urgente_obvia, txt_nombre_iniciativa_urgente_obvia, cmb_tipo_iniciativa_urgente_obvia,
        dtp_fecha_ingreso_iniciativa_urgente_obvia_oficialia_partes, dtp_fecha_sesion_presentacion_iniciativa_urgente_obvia, cmb_cond_adhesion_iniciativa_urgente_obvia,
        dtp_fecha_resolucion_pleno_iniciativa_urgente_obvia, cmb_sentido_resolucion_pleno_iniciativa_urgente_obvia, txt_votaciones_pleno_a_favor_iniciativa_urgente_obvia,
        txt_votaciones_pleno_en_contra_iniciativa_urgente_obvia, txt_votaciones_pleno_abstencion_iniciativa_urgente_obvia, txt_total_votaciones_pleno_iniciativa_urgente_obvia
    };
            ...
            foreach (Control c in controlesAValidar)
            {
                ...
                c.GotFocus += Control_GotFocus_UO;
        ...
        private async void Control_GotFocus_UO(object sender, EventArgs e)
        {
        ...
        private bool IsDuplicateRecord_RegistrosOU(string variable_cmb)
        {
            foreach (DataGridViewRow row in dgv_registros_pl.Rows)
            {
                ...
                string existingId = row.Cells["txt_ID_iniciativa_urgente_obvia"].Value.ToString();
                ...
                try
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(query, conexion))
                    {
                        ...
                        using (SQLiteDataAdapter adapter = new SQLiteDataAdapter(cmd))
                        {
                            ...
                            dgv_registros_UO.DataSource = dataTable;
                        }
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Error al llenar DGV iniciativas urgentes obvias: " + ex.Message);
                }
                finally
                {
                    conexion.Close();
            ...
            using (var connection = new SQLiteConnection(cadena))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        string query = $"INSERT INTO TR_INICIATIVAS_URG_OBVIA ({columns}, fecha_actualizacion,id_legislatura) " +
                            $"VALUES " +
                        ...
                        using (var command = new SQLiteCommand(query, connection, transaction))
                        {
                            ...
                            command.Parameters.AddWithValue("@id_legislatura", txt_ID_iniciativa_urgente_obvia.Text.ToString());
        ...
        private void RecorrerControlesUO(Control control, Dictionary<string, string> data)
        {
            // List of DataGridView names to exclude
            var excludedDataGridViews = new List<string> { "dgv_registros_UO" };
            ...
            foreach (Control c in control.Controls)
            {
                ...
                else if (c is DataGridView dataGridView && !excludedDataGridViews.Contains(dataGridView.Name))
                {
                        ...
                        for (int j = 0; j < dataGridView.Columns.Count; j++)
                        {
                            if (dataGridView.Rows[i].Cells[j].Value != null)
                            {
                                ...
                                if (!string.IsNullOrEmpty(rowValues))
                                {
                                    ...
                                    if (dataGridView.Name == "dgv_per_legis_UO")
                                    {
                                        string idPL2 = txt_ID_iniciativa_urgente_obvia.Text;
                                        ...
                                        using (SQLiteConnection conn = new SQLiteConnection(cadena2))
                                        {
                                            ...
                                            if (j == 0)
                                            {
                                                string query = "INSERT INTO TR_INICIATIVAS_URG_OBVIA (id_legislatura, " +
                                                    "txt_ID_iniciativa_urgente_obvia," +
                                                    "dgv_ID_persona_legisladora_1_UO, " +
                                                    "fecha_actualizacion) " +
                                                 "VALUES " +
                                                 "(@id_legislatura," +
                                                 "@txt_ID_iniciativa_urgente_obvia," +
                                                ...
                                                using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                                                {
                                                    ...
                                                    cmd.Parameters.AddWithValue("@txt_ID_iniciativa_urgente_obvia", idPL2);
                                            ...
                                            if (j == 1)
                                            {
                                                string query = "INSERT INTO TR_INICIATIVAS_URG_OBVIA (id_legislatura, txt_ID_iniciativa_urgente_obvia, dgv_nombre_persona_legisladora_1_UO," +
                                                    "fecha_actualizacion) " +
                                                "VALUES " +
                                                "(@id_legislatura,@txt_ID_iniciativa_urgente_obvia, @RowValue, @fecha_actualizacion)";
                                                using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                                                {
                                                    cmd.Parameters.AddWithValue("@RowValue", rowValues);
                                                    cmd.Parameters.AddWithValue("@txt_ID_iniciativa_urgente_obvia", idPL2);
                                    ...
                                    if (dataGridView.Name == "dgv_com_legis_UO")
                                    {
                                        string idPL2 = txt_ID_iniciativa_urgente_obvia.Text;
                                        string cadena2 = "Data Source=DB_PLE.db;Version=3;";
                                        ...
                                        using (SQLiteConnection conn = new SQLiteConnection(cadena2))
                                        {
                                            ...
                                            if (j == 0)
                                            {
                                                string query = "INSERT INTO TR_INICIATIVAS_URG_OBVIA (id_legislatura,txt_ID_iniciativa_urgente_obvia, dgv_ID_comision_legislativa_1_UO," +
                                                    "fecha_actualizacion) " +
                                                 "VALUES " +
                                                 "(@id_legislatura,@txt_ID_iniciativa_urgente_obvia, @RowValue, @fecha_actualizacion)";
                                            ...
                                            if (j == 1)
                                            {
                                                string query = "INSERT INTO TR_INICIATIVAS_URG_OBVIA (id_legislatura,txt_ID_iniciativa_urgente_obvia, dgv_nombre_comision_legislativa_1_UO," +
                                                    "fecha_actualizacion) " +
                                                "VALUES " +
                                                "(@id_legislatura,@txt_ID_iniciativa_urgente_obvia, @RowValue, @fecha_actualizacion)";
                                                using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                    ...
                    foreach (var rowValue in rowValuesList)
                    {
                        ...
                        string idPL = txt_ID_iniciativa_urgente_obvia.Text;

                        string cadena = "Data Source=DB_PLE.db;Version=3;";
                        using (SQLiteConnection conn = new SQLiteConnection(cadena))
                        {
                            conn.Open();
                            if (dataGridView.Name == "dgv_grupos_parla_UO")
                            {
                                string query = "INSERT INTO TR_INICIATIVAS_URG_OBVIA (id_legislatura, txt_ID_iniciativa_urgente_obvia, dgv_grupo_parlamentario_UO," +
                                    "fecha_actualizacion) " +
                                    "VALUES " +
                                    "(@id_legislatura,@txt_ID_iniciativa_urgente_obvia, @RowValue, @fecha_actualizacion)";
                                ...
                                using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@txt_ID_iniciativa_urgente_obvia", idPL);
                                    cmd.Parameters.AddWithValue("@fecha_actualizacion", DateTime.Now.ToString("yyyy-MM-dd HH:mm"));
                                    cmd.Parameters.AddWithValue("@id_legislatura", txt_id_legislatura.Text.ToString());
        ...
        private void ClearControlsUO(Control control)
        {
        ...
        private void btnActualizarDGV_UO_Click(object sender, EventArgs e)
        {
            DGV_REGISTROS_UO();
        }
